...
export async function createRoleIntakeUpload(
    ...
    data: {
      ...
      events: {
        create: {
          ...
          metadata: roleIntakeSourceSelectedMetadata({
            intakeId: id,
            sourceKind: "file",
          }),
        },
  ...
  try {
    ...
    await failRoleIntake(
      created.id,
      "storage_unavailable",
      "HireCall could not prepare a private upload.",
    );
    return {
      ok: false,
      error: "HireCall could not prepare a private upload. Please retry.",
    };
  }
}
...
export async function finalizeRoleIntakeUpload(
  ...
  try {
    ...
    const queued = await prisma.roleIntake.update({
      data: {
        events: {
          create: [
            {
              eventType: "role_intake_upload_received",
              metadata: {
                intake_id: intake.id,
                size_bucket: sizeBucket(metadata.byteSize),
              },
            },
          ],
        },
    ...
    await cleanupAndFailRoleIntake({
      ...
      message:
        "HireCall could not secure this upload. Please retry with a fresh file.",
      storage,
    });
    return {
      ok: false,
...
export async function saveRoleIntakeReview(
  ...
  let draft: ImportedRoleDraft;
  try {
    draft = normalizeImportedRoleDraft(input.reviewedDraft);
  } catch {
    return {
      ok: false,
      error:
        "Review the role title, location, and job description before continuing.",
    };
  }
  const intake = await prisma.roleIntake.findFirst({
    ...
    data: {
      events: {
        create: {
          ...
          metadata: roleIntakeReviewSubmittedMetadata({
            changedFields: changedRoleDraftFields(intake.reviewedDraft, draft),
            elapsedMs: elapsedMilliseconds(intake.createdAt),
            intakeId: intake.id,
          }),
        },
      },
...
export async function recordRoleIntakeManualFallback(
  scope: CompletedOrganizationScope,
  intakeId: string,
): Promise<RoleIntakeOperationResult<null>> {
  if (!canManageRoleIntake(scope.role)) {
    return {
      ok: false,
      error: "Only recruiters, admins, and owners can manage a role brief.",
    };
  }
  const intake = await prisma.roleIntake.findFirst({
    select: {
  ...
  const reason =
    intake.lastErrorCode === "no_usable_text"
      ? "no_usable_text"
      : intake.lastErrorCode
        ? "processing_failed"
        : "user_selected";
  await recordRoleIntakeEvent(
    intake.id,
    "role_intake_manual_fallback_selected",
    {
      intake_id: intake.id,
      reason,
        ...
        data: {
          events: {
            create: {
              ...
              metadata: {
                elapsed_ms: elapsedMilliseconds(intake.createdAt),
                intake_id: intake.id,
                outcome: "created",
              },
            },
...
export async function processNextRoleIntake(
  ...
  try {
    ...
    if (scan.kind === "unavailable") {
      await recordRoleIntakeEvent(intake.id, "role_intake_scan_completed", {
        duration_ms: elapsedMilliseconds(scanStartedAt),
        intake_id: intake.id,
        outcome: "unavailable",
      });
        ...
        events: {
          create: [
            ...
            },
            {
              ...
              metadata: roleIntakeUploadCompletedMetadata({
            ...
            },
            {
              ...
              metadata: roleIntakeExtractionCompletedMetadata({
                durationMs: elapsedMilliseconds(extractionStartedAt),
                intakeId: intake.id,
                outcome: "ready",
                pageCount: extraction.pageCount,
                parserVersion: extraction.parserVersion,
                textLength: extraction.textLength,
                warningCodes: extraction.warnings.map(
            ...
            },
            {
              ...
              metadata: { intake_id: intake.id, reason: "extracted" },
            },
          ],
        },
  ...
  try {
    ...
    await prisma.roleIntake.update({
      data: {
          ...
          create: [
            ...
            },
            {
              ...
              metadata: roleIntakeExtractionCompletedMetadata({
                ...
                textLength: imported.draft.description.length,
        ...
        sourceIdentity,
        sourceMetadata: toJson({
          acquisition_strategy: imported.acquisitionStrategy,
          citation_urls: imported.citationUrls,
          ...
          data: {
            events: {
              create: {
                ...
                metadata: { intake_id: intake.id, source_kind: "url" },
              },
            },
      ...
      data: {
        ...
        events: deleted
          ? {
              create: {
                eventType: "role_intake_object_deleted",
                metadata: { intake_id: intake.id, reason: "expired" },
              },
            }
          : undefined,
    ...
    data: {
      ...
      events: {
        create: [
          ...(telemetryEvent
            ? [
                typeof telemetryEvent === "string"
                  ? { eventType: telemetryEvent, metadata: {} }
                  : telemetryEvent,
              ]
            : []),
          ...(deleted
            ? [
                {
                  eventType: "role_intake_object_deleted",
                  metadata: { intake_id: intake.id, reason: "failed" },
                },
              ]
            : []),
        ],
    ...
    data: {
      ...
      events: {
        create: {
          ...
          metadata: { error_code: code, intake_id: id, source_kind: "url" },
        },
      },
      lastErrorCode: code,
      lastErrorSummary: summary,
...
function toSummary(intake: RoleIntakeRecord): RoleIntakeSummary {
  return roleIntakeSummarySchema.parse({
    ...
    failureCode: intake.lastErrorCode,
    ...
    sourceRetention:
      intake.sourceKind === "url"
        ? "not_stored"
        : intake.cleanedUpAt &&
            !intake.quarantineObjectKey &&
            !intake.sealedObjectKey
          ? "deleted"
          : "pending_deletion",
...
async function recordRoleIntakeEvent(
  roleIntakeId: string,
  eventType: string,
  metadata: Record<string, boolean | number | string | string[]>,
): Promise<void> {
  await prisma.roleIntakeEvent.create({
    data: {
      eventType,
...
function toSourceProvenance(
  ...
  return {
    acquisitionStrategy: asAcquisitionStrategy(metadata.acquisition_strategy),
    canonicalUrl: intake.canonicalUrl,
    citationUrls: asCitationUrls(metadata.citation_urls),
...
function asCitationUrls(value: unknown): string[] {
  ...
  return value
    .flatMap((candidate) => {
      ...
      try {
        return [normalizeRoleIntakeUrl(candidate).toString()];
      } catch {
        return [];
      }
    })
