...
function buildBrief({ candidateSessionId, criteria, runId }) {
  const evidenceRefs = criteria.map((criterion) => ({
    criterionId: criterion.id,
    eventId: null,
    questionId: "role-skills",
    transcriptTurnId: "turn_role-skills_candidate",
  }));
  const summary = {
    candidateSessionId,
    criteria: criteria.map((criterion) => ({
      ...
      evidence: [
        {
          questionId: "role-skills",
          text: candidateAnswerFor("role-skills"),
          transcriptTurnId: "turn_role-skills_candidate",
        },
      ],
      label: criterion.label,
      rationale:
        "The smoke transcript includes concrete, job-related evidence for recruiter review.",
      status: "Medium",
    })),
    limitations: [
    ...
    pointsToClarify: [
      "Clarify exact ownership of churn or activation metrics in a follow-up.",
    ],
    risks: [
    ...
    strengths: [
      ...
      "Clarity: answers were concise enough for first-screening review.",
    ],
    suggestedNextStep: "to_review",
    summary:
      "Ada Martin completed the Customer Success Manager smoke interview with persisted transcript evidence, answer evaluations, and a recruiter brief ready for human review.",
  };
