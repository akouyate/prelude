...
13. **SUPERSEDED by amendment 22** (US direct sales, decided 2026-08-15): currencies are EUR + USD, enforced as a closed set — `createCreditCheckoutSession` refuses anything outside `{EUR, USD}` with a named error.
...
22. **US direct sales from day one (decided 2026-08-15) — multi-currency EUR + USD.**
    - **Prices:** each pack's Stripe Price carries `currency_options` (EUR default + USD); Checkout picks the buyer's currency from their location and shows the exact amount before payment. USD working values mirror the EUR numerals ($99 / $349 / $1,490 / $2,790) — same "subject to validation" status as the EUR ladder. Sync script creates/rotates Prices with both options; `CreditPack` caches both display amounts (`unitAmountCents` EUR, `unitAmountCentsUsd Int?`). (Task 1)
    - **The lot records what was actually paid:** fulfilment reads `session.currency` + `session.amount_subtotal` (amendment 3 already cross-checks the amount) and writes BOTH onto the `CreditLot` — a USD purchase must never be recorded as EUR. Reporting (`realised €/interview`) converts at read time, never at write time. (Tasks 3, 5)
    - **Buy UI:** a light EUR/USD display toggle defaulting from the browser locale; Checkout remains the authority on the final amount. (Task 8)
    - **Tax:** no US registrations needed to start — Stripe Tax collects only where registered and monitors nexus thresholds; go-live checklist gains "watch the Stripe Tax thresholds dashboard monthly" and "register + enable collection when a state threshold approaches". The EU single-purpose-voucher analysis is VAT-only; US taxability of the credit follows the underlying service state by state, handled by Stripe Tax's product tax code. (Task 10)
    - **Legal posture (go-live items, not code):** ToS get an English version; the B2B-only clause becomes load-bearing (it is what keeps US gift-card/consumer statutes and Québec's consumer rules at distance); one US-counsel question to answer before meaningful US volume — are these credits "prepaid access" under state stored-value/escheat laws, and how is expired value treated. Per-lot `expiresAt` is the compliance lever if any state ever forces a different expiry.
